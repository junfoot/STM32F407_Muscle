...
- AD7606 使用单 5 V 模拟电源；供电电压不等于输入量程。当前 PC5/PD13 两个 RANGE 引脚均为低电平，因此两片均配置为 **-5 V～+5 V**。理想换算为 `电压(V) = int16_adc × 5 / 32768`。
...
串口的 ADC 字段是固件已经换算后的电压，不是原始整数码。原始码先按上述 ±5 V 比例转换，再填入 JustFloat 帧；PC 端应按小端 `float32` 解码，不能把二进制帧当作 ASCII 文本读取。
...
- `ad7606.c`：双 AD7606 GPIO 模拟串行驱动；SCLK 空闲为高，数据在下降沿采样、上升沿移出下一位。
...
- 当前两片 AD7606 的 RANGE 均为低电平（±5 V）；若将任意一片改为高电平（±10 V），必须同步修改对应通道的原始码和滤波值换算比例。
